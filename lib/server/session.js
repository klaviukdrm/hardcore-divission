...
function userSecret() {
    return process.env.USER_SESSION_SECRET || process.env.ADMIN_SESSION_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || 'hd-user-session-fallback-secret-2026';
}

function adminSecret() {
    return process.env.ADMIN_SESSION_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || 'hd-admin-session-fallback-secret-2026';
}
...
export function setUserSession(res, user) {
    ...
    const ttl = ttlSeconds(parseHours(process.env.USER_SESSION_TTL_HOURS, 24 * 14));
    const token = sign(sessionPayload({ sub: user.id, email: user.email, phone: user.phone || user.email, role: 'user' }, ttl), secret);

    // Keep only one session role at a time.
...
export function setAdminSession(res) {
    ...
    const ttl = ttlSeconds(parseHours(process.env.ADMIN_SESSION_TTL_HOURS, 72));
