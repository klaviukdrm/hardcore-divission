...
async function sendTelegramTextPayload(token, chatId, payload) {
    ...
    if (!response.ok) {
        ...
        throw new Error(errorPayload?.description || 'Telegram sendMessage failed');
    }
...
export async function sendTelegramMessage(text) {
    ...
    try {
        return await sendTelegramTextPayload(token, chatId, {
            text,
            parse_mode: 'HTML'
        });
    } catch (htmlError) {
        return sendTelegramTextPayload(token, chatId, {
