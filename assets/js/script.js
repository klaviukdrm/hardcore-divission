let cart = [];
    ...
    const monoPendingInvoiceKey = 'hd_mono_pending_invoice_id';
    const monoPendingReferenceKey = 'hd_mono_pending_reference';
    let monoStatusPollTimer = null;
    ...
    function clearMonoPendingPayment() {
        try {
            localStorage.removeItem(monoPendingInvoiceKey);
            localStorage.removeItem(monoPendingReferenceKey);
        } catch (e) {
            // Ignore storage errors.
        }
    }

    function setMonoPendingPayment(invoiceId, reference) {
        try {
            if (invoiceId) {
                localStorage.setItem(monoPendingInvoiceKey, String(invoiceId));
            }
            if (reference) {
                localStorage.setItem(monoPendingReferenceKey, String(reference));
            }
        } catch (e) {
            // Ignore storage errors.
        }
    ...
    function getMonoPendingPayment() {
        try {
            const invoiceId = localStorage.getItem(monoPendingInvoiceKey) || '';
            const reference = localStorage.getItem(monoPendingReferenceKey) || '';
            return { invoiceId: invoiceId.trim(), reference: reference.trim() };
        } catch (e) {
            return { invoiceId: '', reference: '' };
        }
    }

    function stopMonoStatusPolling() {
        if (monoStatusPollTimer) {
            clearTimeout(monoStatusPollTimer);
            monoStatusPollTimer = null;
        }
    }
    ...
    async function startMonoCheckout() {
        ...
        try {
            ...
            setMonoPendingPayment(payload.invoiceId || payload.order_id || '', publicOrderCode);
    ...
    async function resumeMonoPaymentIfNeeded() {
        const pending = getMonoPendingPayment();
        if (!pending.invoiceId) {
            stopMonoStatusPolling();
            return false;
        }

        try {
            const response = await fetch(`/api/payments/mono/status?invoiceId=${encodeURIComponent(pending.invoiceId)}`, {
                method: 'GET',
                credentials: 'include',
                cache: 'no-store'
            });
            ...
            if (!monoStatusPollTimer) {
                ...
                const poll = async () => {
                    attempts += 1;
                    monoStatusPollTimer = null;
                    const next = await resumeMonoPaymentIfNeeded();
                    if (next || attempts >= 12) {
                        stopMonoStatusPolling();
                        return;
                    }
                    monoStatusPollTimer = setTimeout(poll, 2500);
                };
                monoStatusPollTimer = setTimeout(poll, 2500);
            }
        } catch (e) {
            if (!monoStatusPollTimer) {
                ...
                const poll = async () => {
                    ...
                    monoStatusPollTimer = setTimeout(poll, 3000);
                };
                monoStatusPollTimer = setTimeout(poll, 3000);
            }
        }
