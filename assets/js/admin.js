...
async function updateOrderStatus(orderId, nextStatus, trackingNumber) {
...
function setTrackingVisibility(card) {
    const select = card.querySelector('.admin-status-select');
    const wrap = card.querySelector('.admin-ttn-wrap');
    const input = card.querySelector('.admin-ttn-input');
    if (!select || !wrap || !input) return;

    const isShipping = select.value === SHIPPING_STATUS;
    wrap.hidden = !isShipping;
...
function renderOrders(orders) {
    ...
    adminOrdersList.innerHTML = orders.map((order) => {
        ...
        const isShipping = order.status === SHIPPING_STATUS;
        const trackingValue = order.tracking_number ? escapeHtml(order.tracking_number) : '';
        ...
        return `
            <article class="order-card" data-order-id="${order.id}">
                ...
                <div class="admin-order-actions">
                    ...
                    <div class="admin-ttn-wrap" ${isShipping ? '' : 'hidden'}>
                        <label class="field-label admin-ttn-label">ТТН Нова Пошта</label>
                        <input class="admin-ttn-input" type="text" inputmode="numeric" maxlength="20" placeholder="Наприклад: 20400048799000" value="${trackingValue}">
                    </div>
    ...
    adminOrdersList.querySelectorAll('.admin-status-select').forEach((select) => {
        select.addEventListener('change', () => {
            const card = select.closest('.order-card');
            setTrackingVisibility(card);
        });
    });
    ...
    adminOrdersList.querySelectorAll('.admin-save-btn').forEach((button) => {
        button.addEventListener('click', async () => {
            ...
            const select = card.querySelector('.admin-status-select');
            const ttnInput = card.querySelector('.admin-ttn-input');
            const status = select.value;
            const tracking = ttnInput ? ttnInput.value.replace(/\s+/g, '') : '';

            if (status === SHIPPING_STATUS && !TTN_REGEX.test(tracking)) {
                showMessage('Вкажіть коректний ТТН (8-20 цифр).', true);
                return;
            }
